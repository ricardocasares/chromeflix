...
var requestFileSystem_ = window.requestFileSystem || window.webkitRequestFileSystem;
var PersistentStorage_ = navigator.PersistentStorage || navigator.webkitPersistentStorage;
	...
	/* these are proportional to torrent rate,
	   ...
	   */
	   ...
	   if (!peer.has(piece.pieceNumber)) {
	   	console.warn('Found interesting piece for peer who doesnt have it', peer.ip, piece.pieceNumber);
	   } else {
...
findInterestingPiece: function(hintPeer) {
	...
	for(i = 0; i < idxs.length; i++) {
       ...
       getBytesLeft: function() {
       	...
       	for(var i = 0; i < this.pieces.length; i++) {
       		if (!this.pieces[i].valid) {
       			this.pieces[i].chunks.forEach(chunks);
       		}
       	}
     ...
     */
     consume: function(offset, cb) {
     	...
     	if (piece) {
     		...
     		for(var i = piece.pieceNumber; piecesReadahead > 0 && i < this.pieces.length; i++) {
     			if (!this.pieces[i].valid) {
     				piecesReadahead--;
     				readahead.push(i);
     			}
