function Torrent(meta) {
    ...
    if (typeof meta.info.length instanceof Number) {
        torrentSize = meta.info.length;
        this.files = [{ path: [name], size: meta.info.length }];
    } else if (meta.info.files instanceof Array) {
        torrentSize = 0;
...
Torrent.prototype = {
    start: function() {
        ...
        this.unchokeInterval = setInterval(this.tickUnchoke.bind(this), 10000);
    },
    ...
    connectPeerLoop: function() {
        ...
        if (candidate && connecting < 80 && connected < 60) {
            ...
            candidate.connect();
            upShaper.enqueue({
                amount: 8192,  // one connection attempt ~ 8 KB
            ...
            setTimeout(function() {
                candidate = undefined;
                this.peers.forEach(function(peer) {
                    if (!candidate || peer.downRate.getRate() < candidate.downRate.getRate()) {
                        candidate = peer;
                    }
                });
                if (candidate) {
    ...
    tickUnchoke: function() {
        var byScore = this.peers.filter(function(peer) {
                return (peer.state === 'connected' && !peer.seeder && peer.interested);
